...
module.exports = {
  ...
  initHost: async function (_config, ssh) {
    ...
    console.log("");
    await this.runCommand(`cd ${_config.server.deploymentDir} && mkdir ${_config.basename}`, ssh, "Start create Folder application");

    let cmd = `cd ${_config.server.deploymentDir}  && [ -d "${_config.basename}" ] && echo "yes" || echo "noop"`;
    let result = await this.runCommand(cmd, ssh, "Start check source application");
    if (/.*noop.*/.test(result)) {
  ...
  upToServer: async function (_config, ssh) {
    ...
    try {
      let cwd_process = process.cwd();
      await ssh.putFile(`${cwd_process}/DockerfileProduction`, `${_config.server.deploymentDir}/${_config.basename}/Dockerfile`);
  ...
  existsRemoteImage: async function (_config, ssh) {
    let check_image_already = await this.runCommand(`[ -z "$(docker images -q ${_config.basename})" ] && echo 'N' || echo 'Y'`, ssh, "check image on server");
    console.log(`${_config.basename}`, check_image_already);
  ...
  buildImage: async function (_config, ssh) {
    ...
    cmd = `cd ${_config.server.deploymentDir}/${_config.basename} && ${_config.server.sudo} docker build  -t ${_config.basename} .`;
