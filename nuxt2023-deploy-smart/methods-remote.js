...
module.exports = {
  ...
  initHost: async function (_config, ssh) {
    ...
    console.log("");
    await this.runCommand(`cd ${_config.server.deploymentDir} && mkdir ${_config.appName}`, ssh, "Start create Folder application");
  ...
  upToServer: async function (_config, ssh) {
    ...
    try {
      let cwd_process = process.cwd();
      await ssh.putFile(`${cwd_process}/output.tar.gz`, `${_config.server.deploymentDir}/${_config.appName}/output.tar.gz`);
  ...
  runAppNotBuild: async function (_config, ssh, isRemoteBase) {
    ...
    let cmd = `${_config.server.sudo} docker stop ${_config.appName} && docker rm -f ${_config.appName} || echo 'not' 1>&2 && ${_config.server.sudo} docker run --restart=always --name ${_config.appName} ${network} -d ${_config.volume} ${_config.port} ${_config.basename}`;
