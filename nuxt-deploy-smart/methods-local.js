...
module.exports = {
    ...
    build: async function (deployJsonName = "config.json") {
        ...
        try {
            ...
            let command3 = `cd .. && tar czf ${cwd_process}/static.tar.gz static`;
            await this.runCommand(command3);
            let command4 = `cd .. && cp nuxt.config.js ${cwd_process}/nuxt.config.js`;
    ...
    remove: async function (deployJsonName = "config.json") {
        ...
        if (fs.existsSync(`${cwd_process}/static.tar.gz`)) {
            await this.removeFolder(`${cwd_process}/static.tar.gz`);
        }
