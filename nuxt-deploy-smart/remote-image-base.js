...
module.exports = {
    ...
    existsRemoteImage:async function(_config, ssh){
        ...
        let check_image_already = await this.runCommand(`[ -z "$(docker images -q b-${_config.basename}-${_version_})" ] && echo 'N' || echo 'Y'`,ssh,'check image on server');
        console.log(`b-${_config.basename}-${_version_}`,check_image_already);
    ...
    buildImage: async function (_config, ssh) {
        ...
        let _version_ = fs.readFileSync('image-base-version', 'utf8');
        if(_version_) _version_ = _version_.toString();
        cmd = `cd ${_config.server.deploymentDir}/${_config.basename} && ${_config.server.sudo} docker build  -t b-${_config.basename}-${_version_} .`;
