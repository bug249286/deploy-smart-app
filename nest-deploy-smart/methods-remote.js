...
module.exports = {
    ...
    runAppNotBuild: async function (_config, ssh, isRemoteBase) {
        ...
        if (typeof _config.multiapp !== 'undefined' && _config.multiapp === true) {
            ...
            cmd = `${_config.server.sudo} docker cp ${_config.server.deploymentDir}/${_config.appName}/dist ${_config.appName}:/usr/src/app/dist`;
        }
    ...
    reload: async function (_config, ssh) {
        cmd = `${_config.server.sudo} docker exec -t ${_config.appName} pm2 restart 0`;
        return await this.runCommand(cmd, ssh, "restart success ");
        //return await ssh.exec(`docker exec -t ${_config.appName} pm2 restart 0`,[],{});
        //let cmd = `${_config.server.sudo} docker restart ${_config.appName}`;
        //return await this.runCommand(cmd, ssh, "restart");
    },
